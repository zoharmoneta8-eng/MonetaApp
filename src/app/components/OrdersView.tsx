                  ...
                  <tbody className="divide-y divide-gray-200">
                    {visible.map((item) => (
                      <tr key={item._id || JSON.stringify(item)}>
                        ...
                        <td className="px-6 py-4 text-sm">{item.paid ? "כן" : "לא"}</td>
                        <td className="px-6 py-4 text-sm">{item.paid ? `₪${item.payment || 0}` : "-"}</td>
                        <td className="px-6 py-4 text-sm font-bold text-orange-700">₪{(item.finalPrice || 0).toFixed(2)}</td>
                        <td className="px-6 py-4 text-sm">₪{(item.estimatedPrice || 0).toFixed(2)}</td>
                        <td className="px-6 py-4 text-sm">{item.destination || "-"}</td>
                        <td className="px-6 py-4 text-sm">{item.weight || "-"}</td>
                        <td className="px-6 py-4 text-sm">{item.deliveryNote || "-"}</td>
                        <td className="px-6 py-4 text-sm">{item.size || "-"}</td>
                        <td className="px-6 py-4 text-sm">{item.variety || "-"}</td>
                        <td className="px-6 py-4 text-sm">{item.crates || "-"}</td>
